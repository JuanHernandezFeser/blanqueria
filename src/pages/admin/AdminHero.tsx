...
import type { HeroSlide } from '@/stores/heroStore';
import { api } from '@/services/api';
import { formatPrice } from '@/services/shippingService';
import { Pencil, Trash2, Plus, MoveUp, MoveDown, Upload } from 'lucide-react';
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Dialog open={addOpen} onOpenChange={(v) => { setAddOpen(v); if (!v) resetForm(); }}>
          <DialogTrigger asChild>
            <button className="flex items-center gap-2 rounded-md bg-foreground px-4 py-2.5 text-xs font-medium uppercase tracking-wider text-background font-body hover:opacity-90 transition-opacity">
              <Plus className="h-3.5 w-3.5" /> Agregar imagen
            </button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Agregar imagen al Hero</DialogTitle>
            </DialogHeader>
            <FormContent
              preview={preview}
              title={title}
            ...
            <div key={slide.id} className="rounded-lg border border-accent overflow-hidden">
              <div className="aspect-[16/9] bg-muted relative group">
                ...
                <div className="absolute top-2 left-2">
                  <span className="rounded-full bg-background/80 backdrop-blur-sm px-2.5 py-1 text-[10px] font-body font-medium text-foreground">
                    {slide.type === 'product' ? 'Producto' : 'Imagen'}
                  </span>
                </div>
              </div>
