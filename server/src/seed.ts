...
export function seed(db: Database) {
  ...
  const products = [
    ...
    { id: '19', name: 'Set Individual + Servilletas x4', description: 'Set de 4 individuales y 4 servilletas de algodón. Bordado discreto en el borde. Varios colores disponibles.', brand: 'Blanc Pur', category: 'Manteles', price: 14500, stock: 25, image: 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=600&h=750&fit=crop', colors: ['Natural', 'Gris', 'Terracota'] },
  ...
  for (const p of products) {
    insertProduct.run(
      p.id, p.name, p.description, p.brand, p.category, '', p.price, p.stock, p.image,
      JSON.stringify([]), JSON.stringify(p.variants || []), JSON.stringify(p.colors || []),
      JSON.stringify({}), p.featured ? 1 : 0, p.isNew ? 1 : 0
    );
  }
  ...
  insertSlide.run('default-2', 'image', 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=1400&h=700&fit=crop', 'Colección Otoño', 'Fibras naturales para tu hogar', 1);
  insertSlide.run('default-3', 'image', 'https://images.unsplash.com/photo-1616627561950-9f746e330187?w=1400&h=700&fit=crop', 'Toallas Premium', 'Suavidad en cada detalle', 2);

  db.run('INSERT INTO bank_config (id, bank_name, cbu, alias, account_holder) VALUES (1, ?, ?, ?, ?)', ['Banco Ejemplo', '0000000000000000000000', 'BLANQUERIA.TRANSFERENCIA', 'AIKEN S.A.']);
}
