...
export async function sendOrderConfirmation(order: any) {
  ...
  try {
    ...
    console.log('[mail] Enviando email a', order.customerEmail, 'desde', process.env.EMAIL_FROM || 'onboarding@resend.dev');
    const res = await fetch(RESEND_API, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body,
    ...
    if (!res.ok) {
      ...
      console.log('[mail] Email enviado:', text);
    }
  } catch (err) {
    console.error('[mail] Error al enviar email:', err);
  }
}
...
function internalOrderNotificationHtml(order: any): string {
  const itemsHtml = (order.items || []).map((item: any) => `
    <tr>
      <td style="padding:8px 0;border-bottom:1px solid #e5e7eb;font-size:14px;color:#1f2937">${item.productName}${item.variant ? ` <span style="color:#6b7280;font-size:12px">(${item.variant})</span>` : ''}</td>
      <td style="padding:8px 0;border-bottom:1px solid #e5e7eb;font-size:14px;color:#6b7280;text-align:center">x${item.quantity}</td>
  ...
  const adminUrl = (process.env.SITE_URL || 'http://localhost:8080') + '/admin';
...
export async function sendInternalOrderNotification(order: any) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) { console.log('[mail] RESEND_API_KEY no configurada, email interno no enviado'); return; }
  try {
    const body = JSON.stringify({
      from: process.env.EMAIL_FROM || 'onboarding@resend.dev',
      to: 'compras@aikenblanco.com.ar',
      subject: `Nuevo pedido ${order.id}`,
      html: internalOrderNotificationHtml(order),
    });
    console.log('[mail] Enviando notificación interna de pedido a compras@aikenblanco.com.ar');
    const res = await fetch(RESEND_API, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body,
    ...
    const text = await res.text();
    if (!res.ok) {
      console.error('[mail] Error de Resend al enviar notificación interna:', res.status, text);
    } else {
      console.log('[mail] Notificación interna enviada:', text);
    }
  } catch (err) {
    console.error('[mail] Error al enviar notificación interna:', err);
  }
}

export async function sendOrderStatusUpdateEmail(order: any, newStatus: string) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) { console.log('[mail] RESEND_API_KEY no configurada, email de estado no enviado'); return; }
  ...
  try {
    ...
    console.log('[mail] Enviando email de cambio de estado a', order.customerEmail);
    const res = await fetch(RESEND_API, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body,
    ...
    const text = await res.text();
    if (!res.ok) {
      console.error('[mail] Error de Resend al enviar cambio de estado:', res.status, text);
    } else {
      console.log('[mail] Email de cambio de estado enviado:', text);
    }
  } catch (err) {
    console.error('[mail] Error al enviar email de cambio de estado:', err);
  }
}
